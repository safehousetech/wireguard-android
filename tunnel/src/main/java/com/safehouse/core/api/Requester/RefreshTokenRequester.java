package com.safehouse.core.api.Requester;

import com.safehouse.core.api.ApiResponse;
import com.safehouse.core.api.NetworkClient;
import com.safehouse.core.api.RequestExecutor;
import com.safehouse.core.model.RefreshTokenResponse;

import java.io.IOException;
import javax.inject.Inject;
import javax.inject.Singleton;
import org.json.JSONException;
import org.json.JSONObject;
import retrofit2.Response;

@Singleton
public class RefreshTokenRequester implements RequestExecutor<RefreshTokenResponse> {

  private final NetworkClient networkClient;

  @Inject
  public RefreshTokenRequester(NetworkClient networkClient) {
    this.networkClient = networkClient;
  }

  @Override
  public ApiResponse<RefreshTokenResponse> executeRequest(Boolean isMoreTimeNeeded) {
    try {
      networkClient.setIsMoreTimeoutNeeded(isMoreTimeNeeded);
      //            networkClient.addInterceptor(NetworkingInterface.InterceptorType.SIGNED_APIS);
      Response<RefreshTokenResponse> response =
          networkClient.createService().refreshToken().execute();

      switch (response.code()) {
        case 401:
          return ApiResponse.error("refresh token has expireed", null, 401);
        case 200:
          return ApiResponse.success(response.body(), 200);
        case 404:
          return ApiResponse.error("404 Not Found", "404 Not Found", response.code());
        case 422:
          return ApiResponse.error(
              "404 Not Found", "refresh token is not present in request headers", response.code());
        default:
          JSONObject jObjError = new JSONObject(response.errorBody().string());
          String errorMsg =
              jObjError.getString("message");
          return ApiResponse.error(errorMsg, response.errorBody().toString(), response.code());
      }
    } catch (IOException | JSONException e) {
      e.printStackTrace();
    }
    return null;
  }
}
